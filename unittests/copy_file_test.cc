...
class FatalQcheckIo : public QcheckIo {
 public:
  FatalQcheckIo(const char *condition) { std::cerr << "Check failed: " << condition; }
  ~FatalQcheckIo() {
    std::cerr << "\n";
    std::cerr.flush();
    abort();
  ...
  QcheckIo &operator<<(ssize_t i) override {
    NextOutput();
    std::cerr << i;
    return *this;
  }
  QcheckIo &operator<<(size_t i) override {
    NextOutput();
    std::cerr << i;
    return *this;
  }
...
bool FileCopied(const std::string &a, const std::string &b) {
  ...
  while (true) {
    ...
    if (read_a != read_b) {
      std::cerr << "File " << a << " and file " << b
                << " have different lengths. Read results at offset " << offset << ": a=" << read_a
                << " b=" << read_b << "\n";
      return false;
    }
    if (read_a == 0) {
    ...
    if (std::memcmp(buffer_a, buffer_b, read_a) != 0) {
      std::cerr << "File " << a << " and file " << b
                << " have different contents when reading at offset " << offset << "\n";
      return false;
    }
    offset += read_a;
  }
...
std::pair<std::string, int> CreateFileWithContent(std::string content, const std::string &dir) {
  ...
  QCHECK(t3_widget::nosig_write(name_and_fd.second, content.data(), content.size()) ==
...
void FillWithRandomData(const std::pair<std::string, int> &name_and_fd, size_t size) {
  ...
  while (size > 0) {
    ...
    QCHECK(t3_widget::nosig_write(name_and_fd.second, buffer, to_write) ==
...
TEST_F(CopyFileTest, CopyFileRangeWithContentCrossFs) {
  ...
  EXPECT_EQ(copy_file_by_copy_file_range(src_name_and_fd_.second, dest_name_and_fd_.second, 4),
            EXDEV);
}
...
int main(int argc, char **argv) {
  ...
  QCHECK(!FLAGS_non_reflink_fs_dir.empty())
      << "--non_reflink_fs_dir or NONREFLINK_MOUNT must be set";
  ...
  QCHECK(reflink_stat.st_dev != non_reflink_stat.st_dev)
      << "reflink enabled and non-reflink enabled file systems must be different";
